...
const WatchList = () => {
  ...
  useEffect(() => {
    fetch(serverURL + '/portfolio/' + userId + '/timeline?start=' + selectBasedonInterval() + '&interval=1h').then((res) => res.json()).then(res => {
  ...
  if (portfolioDetails.length > 0) {
    ...
    for (var i = portfolioDetails.length - 1; j < 24; i--, j++) {

      y[j].push(new Date(portfolioDetails[i].timestamp).getHours().toString());
      console.log(y[j][0], "this is what ", new Date(portfolioDetails[i].timestamp));
      y[j].push(portfolioDetails[i].current_price);
      console.log(y[j][0], "time ma", y[j][1], "complete", portfolioDetails[i].timestamp);
      if (StartInterval > y[j][1]) {
        StartInterval = y[j][1]
      }
      if (EndInterval < y[j][1]) {
        EndInterval = y[j][1]
      }
    ...
    for (var i = portfolioDetails.length - 1; j < 24; i--, j++) {
      ...
      console.log("array_2", j, array_2[j])

      array_2[j].push(new Date(portfolioDetails[i].timestamp).getHours().toString());
      array_2[j].push(portfolioDetails[i].invested_price);
      console.log(array_2[j][0], "time ma", array_2[j][1]);
      if (StartInterval > y[j][1]) {
        StartInterval = y[j][1]
      }
      if (EndInterval < y[j][1]) {
    ...
    console.log(array_2, "this is y2")
        ...
        <Card className={classes.headingAlignment}>
          <Grid container className={classes.root2} direction="row">
            ...
            <Grid container item className={classes.root} >
              <Grid container item style={{ display: 'flex', justifyContent: 'space-between'}} >
